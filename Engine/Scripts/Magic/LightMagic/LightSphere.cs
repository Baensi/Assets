...
namespace UnityStandardAssets.Effects {
	public class LightSphere : MonoBehaviour {
		...
		[SerializeField] [Range(0.5f, 1.2f)] public float rndIntensity;
		...
		private void Update() {
			if (m_Burning) {
				m_Light.intensity = rndIntensity + (2f - rndIntensity) * Mathf.PerlinNoise(m_Rnd + Time.time, m_Rnd + 1 + Time.time * 1);
